...
2018/06/03 Kentaro Hase 低気圧抽出の際の不具合を修正し更新
'''
# トラッキングを行う開始年・終了年
syear = 2006
eyear = 2017
...
def grib_download(year):
    ...
    if year < 2014:
        _f1 = ['anl_surf125.002_prmsl.{0}010100_{0}123118'.format(year)]
    elif ((year % 4 == 0) & (year % 100 != 0)) | (year % 400 == 0):
        _f1 = ['anl_surf125.002_prmsl.{0}020100_{0}022918'.format(year),
        ...
        _f1 = ['anl_surf125.002_prmsl.{0}020100_{0}022818'.format(year),
             'anl_surf125.002_prmsl.{0}030100_{0}033118'.format(year),
             'anl_surf125.002_prmsl.{0}040100_{0}043018'.format(year),
             'anl_surf125.002_prmsl.{0}050100_{0}053118'.format(year),
             'anl_surf125.002_prmsl.{0}060100_{0}063018'.format(year)]
    ...
    if ((year % 4 == 0) & (year % 100 != 0)) | (year % 400 == 0):
        _f2 = ['anl_p125.043_relv.{0}020100_{0}022918'.format(year),
             'anl_p125.043_relv.{0}030100_{0}033118'.format(year),
             'anl_p125.043_relv.{0}040100_{0}043018'.format(year),
             'anl_p125.043_relv.{0}050100_{0}053118'.format(year),
             'anl_p125.043_relv.{0}060100_{0}063018'.format(year)]
    else:
        _f2 = ['anl_p125.043_relv.{0}020100_{0}022818'.format(year),
             'anl_p125.043_relv.{0}030100_{0}033118'.format(year),
             'anl_p125.043_relv.{0}040100_{0}043018'.format(year),
             'anl_p125.043_relv.{0}050100_{0}053118'.format(year),
             'anl_p125.043_relv.{0}060100_{0}063018'.format(year)]
    for i in _f2:
        if path.exists("original/" + i):
            ...
            URL = 'http://rda.ucar.edu/data/ds628.0/anl_p125/{0}/{1}'.format(year, i)
            stdout.write('Downloading {0} ...\n'.format(i))
            stdout.flush()
            infile = opener.open(URL)
            outfile = open(i, 'wb')
...
while SY <= EY:
    if 'vor_90' in locals():
        ...
        print('Making higher confidence limit data of 850hPa vorticity...')
        sy = ((SY - 1) // 10 - 3) * 10 + 1
        ey = sy + 29
        sUTC = datetime(sy, 3, 1) - timedelta(hours=30)
        eUTC = datetime(ey, 6, 1, 6) 
        dt = timedelta(hours=6)
        T = int(0)
        vor_size = np.zeros((375, 29, 29))  # 標本サイズ
        vor_sum = np.zeros((375, 29, 29))   # 標本和
        vor_sum2 = np.zeros((375, 29, 29))  # 標本の2乗和

        while sUTC <= eUTC:
            y, m = sUTC.year, sUTC.month
            
            # Data load(850hPa relative vorticity[s^{-1}])
            if m == 2 and T == 0:
                if ((y % 4 == 0) & (y % 100 != 0)) | (y % 400 == 0):
                    grb = pgo("original/anl_p125.043_relv.{0:04d}020100_{0:04d}022918".format(y))
                else:
                    grb = pgo("original/anl_p125.043_relv.{0:04d}020100_{0:04d}022818".format(y))
            elif sUTC == datetime(y, 3, 1):
                grb = pgo("original/anl_p125.043_relv.{0:04d}030100_{0:04d}033118".format(y))
            elif sUTC == datetime(y, 4, 1):
                grb = pgo("original/anl_p125.043_relv.{0:04d}040100_{0:04d}043018".format(y))
            elif sUTC == datetime(y, 5, 1):
                grb = pgo("original/anl_p125.043_relv.{0:04d}050100_{0:04d}053118".format(y))
            elif sUTC == datetime(y, 6, 1):
                grb = pgo("original/anl_p125.043_relv.{0:04d}060100_{0:04d}063018".format(y))
            else:
                pass
            ...
            d = grb.select()[t]
            relv, _, _ = d.data(lat1=20, lat2=55, lon1=120, lon2=155)
            vor_size[T, :, :] += np.ones((29, 29))
            vor_sum[T, :, :] += relv * 1.0e-02
            vor_sum2[T, :, :] += (relv * 1.0e-02) ** 2
            del relv
    ...
    if SY == EY or SY % 10 == 0:
        sy = ((SY - 1) // 10 - 3) * 10 + 1 # 使用する気候値の最初の年
        np.savez_compressed('{0:04d}-vor90.npz'.format(sy), vor90=vor_90)   # 'yyyy-vor90.npz'を作成し，['vor90']にデータを格納
        del sy, vor_90
    else:
        pass
    SY += int(1)
...
while time <= endt:
    ...
    if 'vor90' in locals():
        ...
        sy =  ((y - 1) // 10 - 3) * 10 + 1
        f = '{0:04d}-vor90.npz'.format(sy)
        if path.exists(f):
            vor90 = np.zeros((375, 37, 45))
            vor90[:, 4:-4, 8:-8] = np.load(f)['vor90']  # 緯度方向に南北5度ずつ，経度方向に東西10度ずつ広くとる
        else:
            print('{0:04d}-vor90.npzが作成できていません。'.format(sy))
            exit()  # スクリプトの終了
    lcl_vor = vor90[t].reshape(-1)
    ...
    d = grb.select()[t]
    relv, _, _ = d.data(lat1=15, lat2=60, lon1=110, lon2=165)
    

    # 対象グリッドの気圧が周囲300km以内で最も小さく，かつ海面更正気圧が基準値よりも小さいか850hPaの相対渦度が基準値よりも大きい場合，対象グリッドを低気圧の中心とする。
    for i in range(lat.reshape(-1).shape[0]):
        kyori = dis[i, :]
        reslp = slpd.reshape(-1) * 1.0e-02
        rerelv = relv.reshape(-1)

        loc_lon = lon[kyori == 0]
        loc_lat = lat[kyori == 0]        
        loc_slp = reslp[kyori == 0]
        loc_relv = rerelv[kyori == 0]
        ...
        if (loc_slp == min_slp) & ((loc_slp < lcl_slp[kyori == 0]) | (loc_relv > lcl_relv[kyori == 0])):
    ...
    if time == datetime(y, 6, 1, 6):
        ...
        if y % 10 == 0:
            sy =  ((y - 1) // 10 - 3) * 10 + 1
            f1 = '{0:04d}-slp90.npz'.format(sy)
            remove(f1)
            f2 = '{0:04d}-vor90.npz'.format(sy)
            remove(f2)
            del slp90, vor90
        else:
            pass
...
df_save.to_csv('./tracking-cyclone2.csv')
print("Finish making ./tracking-cyclone2.csv")
...
while num_min <= num_max:
    if df['低気圧番号'][df['低気圧番号'] == num_min].count() == 0:
        ...
        plt.savefig('./track2/track2-{0:05d}.png'.format(num_min))
        plt.close('all')
